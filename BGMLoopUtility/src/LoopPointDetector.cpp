...
LoopPointDetector::LoopPointDetector()
    : loopStart(0), loopLength(0), sameRange(4410) {}
...
bool LoopPointDetector::detectLoopStart(quint32 assumedLoopEnd) {
    ...
    QVector<qint16> samplesToFindL =
        wfd->lChannel.mid(assumedLoopEnd - harfSR, this->sameRange);
    QVector<qint16> samplesToFindR =
        wfd->rChannel.mid(assumedLoopEnd - harfSR, this->sameRange);
    ...
    for (; index < endOfSearch; index++) {
        ...
        for (; rangeIdx < this->sameRange; rangeIdx++) {
            bool cL =
                checkCongruencySample(samplesToFindL[rangeIdx],
                                      wfd->lChannel[index - harfSR + rangeIdx]);
            bool cR =
                checkCongruencySample(samplesToFindR[rangeIdx],
    ...
    // if the program reached this line, that means almost same point is not
    // found
    return false;
}

bool LoopPointDetector::detectLoopStartSemiauto(quint32 assumedLoopEnd,
                                                quint32 skipSample,
                                                quint16 trialNumber) {
    for (quint16 num = 0; num < trialNumber; num++) {
        if (detectLoopStart(assumedLoopEnd)) return true;  // found LOOPSTART
        // if detection failed, move assumedLoopEnd backward by skipSample and
        // retry to detect
