...
void LoopPointDetectorGUI::showEvent(QShowEvent *event) {
    ...
    quint32 sampleLength =
        WaveFormFileManager::getInstance().getWaveFormData()->lChannel.size();
    QString sampleLabelContent = tr("current file's total samples:") + " " +
                                 QString::asprintf("%ld", sampleLength);
    ui->totalSampleLabel->setText(sampleLabelContent);

    // reload WaveFormViewer
    ui->waveFormViewer->reloadWaveFormFile();
}
...
void LoopPointDetectorGUI::startDetection() {
    ...
    if (success) {
        ...
        msgBox.setInformativeText(
            QString("LOOPSTART: ") +
            QString::asprintf("%ld", lpd.getLoopStart()) + "\nLOOPLENGTH: " +
            QString::asprintf("%ld", lpd.getLoopLength()) + "\n" +
...
void LoopPointDetectorGUI::saveLoopInfo() {
    ...
    fileDialog.selectFile(
        WaveFormFileManager::getInstance().getLoopInfoFileName());
    if (fileDialog.exec()) {
